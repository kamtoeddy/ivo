...
console.log(validate.isArrayOk(movieGenres, options)); // { valid: true, validated: ["action", "comedy", "crime", "horror"] }

const invalids = ["   ", [], null, 144];

console.log(validate.isArrayOk(invalids, options)); // { reasons: ["Expected a non-empty array"], valid: false }
```

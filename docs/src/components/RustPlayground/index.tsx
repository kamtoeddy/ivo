...
function loadWasmModule(): Promise<WasmModule> {
  if (!wasmModulePromise) {
    wasmModulePromise = (async () => {
      ...
      const mod = await import('@site/static/wasm/ivo-playground/ivo_playground.js');
      await mod.default();
  ...
  return (
    <div className={styles.playground}>
      <div className={styles.pane}>
        <label className={styles.label} htmlFor={`rust-playground-input-${demo}`}>
          Input (JSON)
        </label>
        <textarea
      ...
      <div className={styles.pane}>
        <div className={styles.outputHeader}>
          ...
          <button className="button button--primary button--sm" onClick={run} disabled={running}>
            {running ? 'Running…' : 'Run'}
          </button>
        </div>

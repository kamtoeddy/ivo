...
| --------- | --------------- | --------------------------------------------------------------------------------------- |
| filter    | function        | A sync or async function to filter the array              |
| modifier  | function        | A sync or async function to modify (format) individual values. Default: **undefined**   |
| postModFilter  | function        | A sync or async function to filter out modified values. Default: **undefined**   |
| map  | function        | A sync or async function to transform modified values after final filter is run. Default: **undefined**   |
| sort    | boolean \| function         | A boolean used to determine whether array should be sorted or not. This value can also be a function that will be used for sorting. Default: **true**                                       |
| sortOrder | 'asc' \| 'desc' | Order used to do comparison check when sort: true and sorter: undefined               |

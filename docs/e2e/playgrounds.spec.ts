...
test.describe('interactive playgrounds', () => {
  test('Rust playground runs the constants demo', async ({ page }) => {
    ...
    const output = page.locator('pre').filter({ hasText: 'username' }).first();
    await expect(output).toContainText('"username": "john-doe"', { timeout: 10_000 });
  });
  ...
  test('TypeScript Sandpack loads and runs on a v2 page', async ({ page }) => {
    ...
    const consolePanel = page.locator('.sp-console, [class*="console"]').first();

...
pub fn create(
    &self,
    input: impl Into<PartialUserInput>,
...
- `update` takes the existing `User` and a `PartialUserInput` of desired changes.
- It returns a `PartialUser` containing only the output fields whose values actually changed.

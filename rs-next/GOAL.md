...
| `#[post_validate([...], validate = ..., pre_validate = ...)]` | ≥2           | `#[lax]`, `#[required]`, `#[ivo_virtual]`                                       | Cross-field validation running after `re_validate`. Both handlers return `Result<Option<PartialInput>, InputErrors>`; `Ok(Some(partial))` applies updates to the built output/input and refreshes the context, `Err(errors)` merges the errors into the payload returned by `create`. `pre_validate` runs before `validate` and can feed updated values into it. Errors are typed via a generated `{InputName}Errors` struct. Currently runs on `create` only. |

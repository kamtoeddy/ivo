...
| `#[post_validate([...], \|ctx, opts\| ...)]`           | ≥2           | `#[lax]`, `#[required]`, `#[ivo_virtual]`                                       | Cross-field validation running after `re_validate`. Handler returns `Result<Option<PartialInput>, InputErrors>`; `Ok(Some(partial))` applies updates to the built output/input, `Err(errors)` merges the errors into the payload returned by `create`. Errors are typed via a generated `{InputName}Errors` struct. Currently runs on `create` only; `pre_validate` builder-style chaining is not yet supported. |

use ivo::{constant_field, lax_field, required_field, IvoInputStruct, IvoModel, IvoStruct};
use std::future::ready;

#[test]
#[should_panic(expected = "[id]: occurs more than once, please remove duplicates")]
fn should_reject_if_field_name_is_already_set() {
    ...
    let _: IvoModel<DataInput, Data> = IvoModel::new(
        |f| {
            f.field(constant_field("id").value(1))
                .field(constant_field("id").value(1))
        },
        |o| o,
    );
}
...
fn should_reject_if_field_name_is_same_created_at_if_enabled_with_default_name() {
    ...
    let _: IvoModel<DataInput, Data, Option<()>, &'static str> = IvoModel::new(
        |f| {
            f.field(constant_field("created_at").value_fn(|_, _| ready(1234)))
...
fn should_reject_if_field_name_is_same_created_at_if_enabled_with_custom_name() {
    ...
    let _: IvoModel<DataInput, Data, Option<()>, &'static str> = IvoModel::new(
        |f| {
            f.field(constant_field("custom_created_at").value(1234))
                .timestamps(|t| {
                    t.resolve(|| "Date.now()")
...
fn should_reject_if_field_name_is_same_updated_at_if_enabled_with_default_name() {
    ...
    let _: IvoModel<DataInput, Data, Option<()>, &'static str> = IvoModel::new(
        |f| {
            f.field(constant_field("updated_at").value_fn(|_, _| ready(1234)))
...
fn should_reject_if_field_name_is_same_updated_at_if_enabled_with_custom_name() {
    ...
    let _: IvoModel<DataInput, Data, Option<()>, &'static str> = IvoModel::new(
        |f| {
            f.field(constant_field("custom_updated_at").value("value".to_string()))
                .timestamps(|t| {
                    t.resolve(|| "Date.now()")

...
describe("constants", () => {
  describe("valid", () => {
    ...
    beforeAll(async () => {
      ...
      >((b, m) =>
        b
          .field(m.constant("asyncConstant", asyncSetter))
          .field(
            m.constant("id", (ctx) =>
    ...
    it("should accept constant(true) & value(any | ()=>any)", () => {
      ...
      for (const value of values) {
        const toPass = makeFx((b, m) =>
          b.field(m.constant("fieldName", value)),
        );
    ...
    it("should accept constant & value + onDelete(function | function[])", () => {
      ...
      for (const onDelete of values) {
        const toPass = makeFx((b, m) =>
          b.field(m.constant("fieldName", "").onDelete(onDelete as never)),
        );
    ...
    it("should accept constant & value + onSuccess(function | function[])", () => {
      ...
      for (const onSuccess of values) {
        const toPass = makeFx((b, m) =>
          b.field(m.constant("fieldName", "").onSuccess(onSuccess as never)),
        );
  ...
  describe("invalid", () => {
    it("should reject constant & value(undefined)", () => {
      const toFail = makeFx((b, m) =>
        b.field(m.constant("fieldName", undefined as never)),
      );

  ...
  describe("valid", () => {
    it("should accept ignore + default", () => {
      const fxn = makeFx((b, m) =>
        b.field(m.lax("fieldName", true).ignore(() => false)),
      );
  ...
  describe("invalid", () => {
    it("should reject ingnore !(() => boolean)", () => {
      ...
      for (const ignore of values) {
        const fxn = makeFx((b, m) =>
          b.field(m.lax("fieldName", true).ignore(ignore as never)),
        );
...
describe("ignoreInit", () => {
  const Model = new Schema<any>((b, m) =>
    b
      .field(m.lax("isBlocked", false).ignoreInit())
      .field(m.lax("env", "dev"))
      .field(m.lax("laxField", 0)),
  ).getModel();
...
describe("ignoreUpdate", () => {
  ...
  describe("behaviour when ignoreUpdate method returns nothing", () => {
    const Model = new Schema<any>((b, m) =>
      b
        .field(m.lax("isBlocked", false).ignoreUpdate())
        .field(m.lax("laxField", 0)),
    ).getModel();
